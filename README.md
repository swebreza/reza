...
| Fallback ingestion | GUI or unsupported tools can drop `.md`/`.json` exports into `.reza/handoffs/` |
| Privacy | Built-in redaction covers common assignment-style secrets and token formats before storage |
| Planned integrations | First-class Codex plugin, MCP server, and richer VS Code UI are intended to call the CLI/API rather than write SQLite directly |
...
For tools without a stable local transcript path, export the chat into `.reza/handoffs/` and keep `reza watch` running. Reza will ingest the drop-zone file and make it searchable through the same commands.
